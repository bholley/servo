...
impl<'ln> GeckoNode<'ln> {
    ...
    unsafe fn from_ref(n: &RawGeckoNode) -> GeckoNode<'ln> {
        GeckoNode::from_raw(transmute(n))
    }

    fn get_node_data(&self) -> NonOpaqueStyleData {
        unsafe {
            transmute(Gecko_GetNodeData(self.node))
        }
    }
}

impl<'ln> Copy for GeckoNode<'ln> {}
impl<'ln> Clone for GeckoNode<'ln> {
    fn clone(&self) -> Self {
        unsafe { GeckoNode::from_raw(self.node) }
    }
}
...
impl<'ln> TNode<'ln> for GeckoNode<'ln> {
    ...
    fn initialize_data(self) {
        unsafe {
            if self.get_node_data().is_null() {
                ...
                let opaque: *mut ServoNodeData = transmute(ptr);
                Gecko_SetNodeData(self.node, opaque);
            }
        }
    }
    ...
    fn as_element(&self) -> Option<GeckoElement<'ln>> {
        if self.is_element() {
            unsafe { Some(GeckoElement::from_raw(transmute(self.node))) }
        } else {
            None
        }
    }
...
impl<'ld> Copy for GeckoDocument<'ld> {}
impl<'ld> Clone for GeckoDocument<'ld> {
    fn clone(&self)-> Self {
        unsafe { GeckoDocument::from_raw(self.document) }
    }
}
...
impl<'ld> TDocument<'ld> for GeckoDocument<'ld> {
    ...
    fn as_node(&self) -> GeckoNode<'ld> {
        unsafe { GeckoNode::from_raw(transmute(self.document)) }
    }
...
impl<'le> GeckoElement<'le> {
    ...
    unsafe fn from_ref(el: &RawGeckoElement) -> GeckoElement<'le> {
        GeckoElement::from_raw(transmute(el))
    }
}

impl<'le> Copy for GeckoElement<'le> {}
impl<'le> Clone for GeckoElement<'le> {
    fn clone(&self) -> Self {
        unsafe { GeckoElement::from_raw(self.element) }
    }
}
...
impl<'le> TElement<'le> for GeckoElement<'le> {
    ...
    fn as_node(&self) -> Self::ConcreteNode {
        unsafe { GeckoNode::from_raw(transmute(self.element)) }
    }
    ...
    fn get_state(&self) -> ElementState {
        unsafe {
            transmute(Gecko_ElementState(self.element))
        }
    }
...
unsafe fn reinterpret_string<'a>(ptr: *const ::libc::c_char, length: u32) -> Option<&'a str> {
    ptr.as_ref().map(|p| from_utf8_unchecked(slice::from_raw_parts(transmute(p), length as usize)))
}
